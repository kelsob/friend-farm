...
func _on_text_completed():
	...
	if current_event.has('max_quantity'):
		set_state(state.WAITING_INPUT)
	
		var waiting_until_options_enabled = float(settings.get_value('input', 'delay_after_options', 0.1))
		$OptionsDelayedInput.start(waiting_until_options_enabled)
		add_numerical_choice_button()
...
func add_numerical_choice_button():
	var numerical_choice_scene = load("res://DialogicHomebrew/NumericalChoiceButton.tscn").instance()
	add_child(numerical_choice_scene)
	numerical_choice_scene.max_count = Dialogic.get_variable("PurchaseItemQuantityAvailable")

	numerical_choice_scene.connect('decision_made', self, '_numerical_decision_made')
	numerical_choice_scene.connect('decision_cancelled', self, '_numerical_decision_cancelled')
	numerical_choice_scene.set_process(true)

func _numerical_decision_made(quantity_purchased):
	pass

func _numerical_decision_cancelled():
	pass
...
# Custom Functions
func numerical_input_functionality():
	set_state(state.TYPING)
	emit_signal("event_start", "question", current_event)
	if fade_in_dialog():
		yield(get_node('fade_in_tween_show_time'), 'tween_completed')
	if current_event.has('name'):
		update_name(current_event['name'])
	elif current_event.has('character'):
		var character_data = DialogicUtil.get_character(current_event['character'])
		update_name(character_data)
		grab_portrait_focus(character_data, current_event)
		...
		if character_data.get('data', {}).get('theme', '') and current_theme_file_name != character_data.get('data', {}).get('theme', ''):
			...
			current_theme = load_theme(current_default_theme)
	#voice 
	handle_voice(current_event)
	update_text(current_event['text'])
