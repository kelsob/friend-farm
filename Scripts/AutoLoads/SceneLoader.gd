		...
		if scene_queue.size() > 0:
			for i in scene_queue:
				...
				if err == ERR_FILE_EOF:
					...
					scene_queue[i].instance = scene_queue[i].loader.duplicate()
					call_deferred("emit_signal", "on_scene_loaded", scene_queue[i])

					
					print(err, ERR_FILE_EOF)
					cache[scene_queue[i].path] = scene_queue[i]
#					emit_signal("on_scene_loaded", scene_queue[i])

					print('scene_loaded 1')
		...
		for awaiter in awaiters:
			if cache.has(awaiter.path):
				if awaiter.path == cache[awaiter.path].path:
					...
					emit_signal("on_scene_loaded", awaiter)
					print('scene_loaded 2')
...
func load_scene(path, props = null):
	...
	if cache.has(path):
		...
		print('scene_loaded 3')
		emit_signal("on_scene_loaded", { path = path, loader = cache[path].loader, instance = cache[path].loader.instance(), props = props })
		return
